...
const LibraryCard = React.forwardRef(function LibraryCard({ item, isOwned, isEquipped, canAfford, isAdmin, busy, onBuy, onEquip, onUnequip }, ref) {
...
function EquippedStrip({ equip }) {
  const equippedItems = Object.entries(equip)
    .map(([type, id]) => LIBRARY_CATALOG.find(i => i.id === id))
    .filter(Boolean);
  ...
  return (
    <div className="flex items-center gap-2 px-5 py-2 flex-shrink-0"
      ...
      <span className="text-[9px] uppercase tracking-widest font-semibold mr-1"
      ...
      {equippedItems.map(item => {
        ...
        return (
          <div key={item.id} className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg"
            ...
            <span className="text-[8px] px-1 py-0.5 rounded"
              style={{ background: `${meta.color}15`, color: `${meta.color}99` }}>
              {meta.label}
            </span>
          </div>
        );
