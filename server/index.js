...
/** Delete update files whose version is older than the latest. Keeps .sig files for the latest version. */
function cleanupOldUpdates() {
  const latest = detectLatestVersion();
  if (!latest) return;
  const latestParts = parseVersion(latest);
  const latestStr = latestParts.join(".");
  let removed = 0;
  for (const f of listUpdates()) {
    const m = f.match(/(\d+\.\d+\.\d+)/);
    if (!m) continue;
    const vParts = parseVersion(m[1]);
    const vStr = vParts.join(".");
    if (vStr === latestStr) continue;
    const older = vParts[0] < latestParts[0]
      || (vParts[0] === latestParts[0] && vParts[1] < latestParts[1])
      || (vParts[0] === latestParts[0] && vParts[1] === latestParts[1] && vParts[2] < latestParts[2]);
    if (!older) continue;
    try {
      fs.unlinkSync(require("path").join(updatesDir, f));
      removed++;
    } catch {}
  }
  if (removed) {
    _updatesCache = null;
    console.log(`[updates] cleanup: removed ${removed} old files (latest=${latest})`);
  }
}
